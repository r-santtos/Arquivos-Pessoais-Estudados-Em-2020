...
function Report() {
  ...
  useEffect(() => {
    db.transaction(
      tx => {
        /** ATIVES MONTH*/
        tx.executeSql("select id, sum(money) as total from ativespassives where month = ? and year = ? and tipos = ?", [varMonth, varYear, 'ativo'], (_, { rows: { _array } }) => setAtives(_array));

        /** PASSIVES MONTH*/
        tx.executeSql("select id, sum(money) as total from ativespassives where month = ? and year = ? and tipos = ?", [varMonth, varYear, 'passivo'], (_, { rows: { _array } }) => setPassives(_array));
  ...
  const createTwoButtonAlert = (delId) =>
    Alert.alert(
      ...
      [
        ...
        { text: "Sim", onPress: () => delCancel(delId) }
      ],
      { cancelable: false }
    );
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <View style={styles.balance}>
        <Text style={[styles.txtWhite, styles.txtMonth]}>Saldo/{months[varMonth-1]}</Text>
        <CalcMonth />
      </View>

      {/** COMPONENT TITLE */}
      <View style={styles.title}>
        <Text style={styles.txtWhite}>Registros</Text>
      </View>
